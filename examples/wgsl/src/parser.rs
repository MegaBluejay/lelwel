...
impl Parser<'_> {
    ...
    fn is_func_call(&self) -> bool {
        matches!(self.tokens[self.pos + 1], Token::LPar | Token::Lt)
            && self.tokens[self.pos + 2] != Token::Lt
    }
}
...
impl<'a> ParserCallbacks<'a> for Parser<'a> {
    ...
    fn predicate_global_directive_1(&self) -> bool {
        self.tokens[self.pos + 1] != Token::Semi
    }
    ...
    fn action_expr_template_list_1(&mut self, diags: &mut Vec<Diagnostic>) {
        if self.current(enumset::EnumSet::empty(), diags) == Token::Lt {
            self.find_template_list();
        }
    }
    ...
    fn predicate_template_args_1(&self) -> bool {
        self.tokens[self.pos + 1] != Token::Gt
    }
    ...
    fn predicate_statement_1(&self) -> bool {
        self.tokens[self.pos + 1] == Token::If
    }
    ...
    fn predicate_case_selectors_1(&self) -> bool {
        !matches!(
            ...
            Token::At | Token::Colon | Token::LBrace
        )
    }
    ...
    fn action_let_decl_1(&mut self, diags: &mut Vec<Self::Diagnostic>) {
        if self.active_error() && self.current(enumset::EnumSet::empty(), diags) == Token::Semi {
            diags
                .last_mut()
                .unwrap()
                .notes
