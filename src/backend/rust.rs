...
impl RustOutput {
    ...
    fn gen_generated(
        ...
        for rule in sema.parts.iter() {
            source.push_str(&Self::gen_parts(cst, *rule).to_string());
        }
        for rule in file.rule_decls(cst) {
            source.push_str(&Self::gen_rule(cst, sema, rule, &token_symbols).to_string());
        }
        source.push_str("}\n\n");
        source.push_str(&Self::gen_parser_callbacks(sema, true, rule_names).to_string());

        source.parse().expect("generated code should be valid Rust")
    }
